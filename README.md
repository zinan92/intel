...
LLM generation uses DeepSeek first. If DeepSeek fails for any reason (including
an exhausted balance), the newsletter retries the same frozen prompt through
the local Codex CLI in an ephemeral read-only process with external tools and
live search disabled. If both providers fail, no previous brief is delivered;
the run is recorded as failed and the Feishu send is skipped.
